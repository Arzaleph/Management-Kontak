...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        $_SESSION['contacts'][$id] = ['nama' => $nama, 'email' => $email, 'telepon' => $telepon];
        header("Location: index.php");
        exit;
    }
}
...
<body class="bg-gray-100 p-8">
    <div class="max-w-md mx-auto bg-white rounded-2xl shadow-lg p-6">
        <h1 class="text-xl font-bold mb-4 text-center">Edit Kontak</h1>
        <?php if (!empty($errors)): ?>
        ...
        <form method="POST" class="space-y-4">
            <div>
                ...
                <input type="text" name="nama" value="<?= htmlspecialchars($contact['nama']); ?>" class="w-full border p-2 rounded">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="telepon" value="<?= htmlspecialchars($contact['telepon']); ?>" class="w-full border p-2 rounded">
            </div>
